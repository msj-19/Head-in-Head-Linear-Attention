...
def gated_chunk_delta_rule_bwd_kernel_dqkw(
    ...
    b_dg -= tl.sum(tl.reshape(b_w*b_dw,(BT,r*BK)),-1)###MAYBE BUG 
...
def gated_bwd_prepare_wy_repr_kernel(           
    ...
    for i_v in range(tl.cdiv(V, BV)):#分块r 
        ...
        sum_dv = tl.sum(b_dv_beta,1)#BT BV
        b_dv = (sum_dv * b_beta[:, None])#BT BV
    ...
    b_dA2 = tl.where(da_mask, -b_dA2, 0) #到这计算结果都一样了
    ...
    b_dA2 *= b_A #BT BT r r
    # b_dA_reshaped = tl.sum(tl.reshape(b_dA2,(BT,BT,r*r)),-1)###BUG Idont know why
    b_dA_reshaped = tl.sum(tl.sum(b_dA2,axis=-1),axis=-1)
    b_dg = tl.sum(b_dA_reshaped,1)-tl.sum(b_dA_reshaped,0)
