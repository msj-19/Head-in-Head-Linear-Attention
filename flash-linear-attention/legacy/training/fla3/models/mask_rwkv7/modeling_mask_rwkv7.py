...
from ...layers.mask_rwkv7 import mask_RWKV7Attention
from ...models.mask_rwkv7.configuration_mask_rwkv7 import mask_RWKV7Config
...
class mask_RWKV7FeedForward(nn.Module):
...
class mask_RWKV7Block(nn.Module):

    def __init__(
        self,
        config: mask_RWKV7Config,
        layer_idx: int
    ) -> mask_RWKV7Block:
        super().__init__()
        ...
        self.ffn = mask_RWKV7FeedForward(
            hidden_size=config.hidden_size,
...
class mask_RWKV7PreTrainedModel(PreTrainedModel):

    config_class = mask_RWKV7Config
    base_model_prefix = 'model'
    supports_gradient_checkpointing = True
    _no_split_modules = ['mask_RWKV7Block']
...
class mask_RWKV7Model(mask_RWKV7PreTrainedModel):

    def __init__(self, config: mask_RWKV7Config):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size

        self.embeddings = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList([mask_RWKV7Block(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
...
class mask_RWKV7ForCausalLM(mask_RWKV7PreTrainedModel, GenerationMixin):

    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = mask_RWKV7Model(config)
        self.vocab_size = config.vocab_size
