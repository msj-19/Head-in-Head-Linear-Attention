...
def intra_chunk_preprocess_fwd_kernel(
    ...
    i_t, i_nh = tl.program_id(0), tl.program_id(1)#chunk idx   nh idx  需要load qk, A shape h->h*r 
    ...
    b_qw = tl.where(m_t, tl.dot(b_q, tl.trans(b_w)), 0).to(b_q.dtype)###分头影响这里
    b_qwT = tl.dot(b_qw, b_T).to(b_q.dtype)###需要拆分q的维度，同时运算 T*d @ T*T   ==> T*d//r @ T*T 的运算 可以同时在r维度并行计算

    b_wbk = tl.where(o_i[:, None] > o_i[None, :], tl.dot(b_w_beta, b_kt), 0).to(b_q.dtype)
    b_A = tl.where(m_t, tl.dot(b_q, b_kt) - tl.dot(b_qwT, b_wbk), 0)###intra head内的Aij 考虑额外的维度操作

    b_q = b_q - tl.dot(b_qwT, b_w_beta)#左q
    ...
    ###在这里重新，处理同一个头的不同分块block的计算exp
    b_qkT_softmax = tl.where(o_i[:, None] >= o_i[None, :], b_A * sm_scale, float("-inf"))
    m_i = tl.max(b_qkT_softmax, 1)##行最大值
    b_qkT_softmax = tl.math.exp2(b_qkT_softmax - m_i[:, None]) #exp运算，这个实际上需要在维度r的并行
    l_i = tl.sum(b_qkT_softmax, 1)#保留用于归一化的li
